...
`0.1.1-rc.59` 按 PVE 8 stable 与 PVE 9 master 的实际注册 API 修正 QGA `agent/exec` 参数编码：Agent 先用最小只读 `/version` 严格解析主版本，并仅成功缓存至多一分钟；未知版本或读取失败绝不发送 guest-exec，也不会根据一次失败猜测或重试另一套编码。PVE 8 与 PVE 9 都以重复 `command` 表单键传递完整固定 argv，随后继续以返回的 PID 轮询状态；两套都不得发送未注册的 `extra-args`、`synchronous` 或 QGA 内部的 `capture-output`、`timeout`、`pass-stdin`。`exec-status` 的 `out-data` 已由 PVE REST 解码，时区观察直接严格校验该明文，绝不二次解码。该版本同时让只读 `vm.inspect-timezone` 在实际执行前严格等待 `guest-exec` ready；超时返回可重试的 `QGA_UNAVAILABLE` 且绝不发送 guest-exec，因此不会为后续网络变更建立错误基线。来宾输出仍只在本地严格校验，绝不进入回执或日志。
...
`0.1.13` 修复 PVE 8/9 的 `/etc/pve/qemu-server` 是 pmxcfs 官方兼容链接时，升级最终模板复核误报 `not a directory` 的问题；仅严格接受 `nodes/<node>/qemu-server` 布局并逐级拒绝其他或嵌套链接，不放宽通用敏感文件读取规则。

`0.1.12` 修复签名远程升级的 systemd 触发竞态：候选包先写入不可触发的 prepared 状态，只有 `AGENT_UPGRADE_SUBMITTED` 已持久化后才原子激活 root helper；旧版遗留的、确定尚未越过 helper 变更门槛的 `COMMAND_STARTED` 会在重启后终结为准备失败并释放节点队列。远程升级的 root 后处理现在也执行旧模板时区迁移，不再错误依赖仅由完整安装调用的 `install.sh`。`0.1.11` 保留 `0.1.10` 的 Cloud-Init 时区修复，同时把旧模板迁移入口并入所有旧版安全升级器已经允许的 `verify-template-bundle.py`，确保 0.1.8/0.1.9 可以直接升级，不会因 Release 新增文件而在安装前失败。`0.1.10` 修复旧 PPFlight 模板的 Cloud-Init vendor-data 固定写入 `timezone: UTC`，导致区域时区在重启后被覆盖并使交付验证失败的问题。每次安装或升级会在停止控制执行后，自动校验模板与内容寻址 snippet 的 SHA-256，新建一份不固定时区的不可变 snippet，再原子切换模板引用；原文件保留，批量切换失败会回滚。`vm.set-timezone` 现在也必须先等待本次 Cloud-Init 进入终态，再设置并读回时区。`0.1.9` 修复 `vm.verify-delivery` 时区不匹配且实际 UTC 偏移为零时，Agent 错误省略 `observedOffsetSeconds`，导致官网拒收终态回执并把开通队列停留在 `COMMAND_STARTED` 的问题。失败回执现在始终输出完整、固定的时区诊断结构。`0.1.8` 为全部只读控制命令增加四分钟整单执行期限，并在执行异常或 Agent 重启后把已经回报 `COMMAND_STARTED` 的只读命令恢复为签名终态；`vm.verify-delivery` 使用官网可安全重试的 `DELIVERY_NOT_READY` 结果，不再占用节点开通队列直至三十分钟命令过期。修改型任务仍不自动重放，未知结果继续按 `EXECUTION_INDETERMINATE` 失败关闭。`0.1.7` 将签名升级的主机防火墙 transient worker 预算从 50 秒提高到 110 秒，覆盖慢速 PVE 节点上的 UFW 精确清理和最终回验；总 helper 与独立回滚仍严格落在旧 180 秒 unit 边界内。失败诊断在固定 512 字节内同时保留受控输出的开头和结尾，不再让前置 INFO 日志截掉真正的 systemd/UFW 终态。`0.1.6` 在每次完整安装和签名升级中先建立、回验 PVE 主机防火墙，再精确移除 UFW 包、服务及其专属 netfilter 规则；升级 helper 同时加入旧 `0.1.5` unit 的 180 秒兼容预算、同版本防火墙收尾以及签名公钥失败恢复。`0.1.5` 兼容 PVE 8/9 网络 reload 的异步 UPID 与同步 JSON `null` 返回；同步完成仍必须通过 active 配置、pending 文件和内核 bridge 的严格回验。模板选择同时接受英文逗号、中文逗号、中文顿号及空格。`0.1.4` 修复重装递增 VM generation 后，已由官网真实状态恢复完成的电源命令仍永久占用 Agent Journal 资源锁的问题。已迁移且身份完全一致的原始 clone 可作为后续 generation 的祖先证明；退役仍仅接受明确列出的、无 PVE UPID 的不确定记录，`PVE_ACTION_INDETERMINATE` 只允许受控电源动作，跨实例、跨绑定及反向代际继续拒绝。

`0.1.7` 同时保留官网所需的逐次升级进度回执，但 monitoring audit 只上传一次已提交阶段和最终结果，不再把每轮 `AGENT_UPGRADE_WAITING` 轮询重复记为监控日志。
...
`0.1.1-rc.49` 把官网控制任务默认领取间隔从 30 秒缩短到 5 秒。升级安装会把旧版精确默认值 `30s` 写成 `5s`；官网签名自升级只替换二进制、无法重写 root 配置，因此新 Agent 启动时还会在内存中完成同一兼容迁移并记录日志。其他管理员自定义值保持不变，从而缩短由多个串行 Agent 命令组成的防火墙与 VPS 生命周期任务等待时间。单 Agent 按常态约 350 台、设计上限 384 台 VPS 规划：CPU、内存、磁盘和流量继续通过一次 `/cluster/resources` 批量采集；逐 VM 的配置与 QGA 读取则按 `sampleInterval` 均匀分片，在一个 `guestInterval` 内完成全量轮转。默认 10 秒/2 分钟配置下，384 台被拆为每批 32 台、共 12 批，避免周期性集中访问全部 VM。该版本还为主 IP 的 make-before-break 切换增加只读交付验证模式：客体网络必须已经使用新地址，而 PVE IPFilter 可在提交前精确包含同地址族的一条旧地址；普通创建、重装和最终交付仍坚持地址与 IPFilter 完全相等。

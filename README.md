...
- 只读发现/回验：`pve.discover`、`firewall.guest.verify-ipfilter`。
...
4. 在 guest 启动前用只读 `firewall.guest.verify-ipfilter` 精确回验 cluster/guest firewall、`ACCEPT/ACCEPT`、MACFilter 与每个集合的正向 host CIDR；若 cluster firewall 未开启则停止交付，由独立审批处理全局变更。

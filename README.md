...
当前 53 个 known actions 已由一致性测试锁住 registry、strict validator 和 Executor 分派。动作存在也不代表官网已批准生产路由；production 仍受签名、assignment、allowlist、审批、资源锁、产品 rollout 和 `productionExecution` 共同限制。`agent.upgrade` 仅接受官网固定 manifest 制品并由独立 root helper 复验、原子替换、回验和回滚，完整合同见[安全自升级合同](docs/SELF-UPGRADE-V1.md)。新增初次定型、固定模板重装、短时控制台、VM 级 legacy Journal 恢复与防火墙规则严格回验合同见[Provisioning actions v1](docs/PROVISIONING-ACTIONS-V1.md)。

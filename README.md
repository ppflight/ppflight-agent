...
模板向导会依次选择模板、镜像缓存 storage、模板磁盘 storage、备份策略与备份 storage、外网桥，以及可选的独立内网桥。外网桥固定用于模板 `net0`，启用内网时内网桥固定用于 `net1`；两者必须存在且不能相同，单网卡环境可明确不创建 `net1`。模板备份默认 `n`。若 active/enabled 存储仅缺受支持的 content 类型，向导会用中文分块显示当前能力、需要新增的能力、完成后的能力和固定 `pvesm set` 命令；只有操作者输入 `y` 才执行，并在继续前重新 discovery 验证。随后先输出无副作用 plan，只有操作者输入 `y` 才按该 plan 的 `requestId/operationId/catalogRevision/catalogSha256` 执行；输入 `n` 或直接回车会取消。全部系统镜像固定到已审定的官网日期/构建路径，不使用会漂移的 `latest`；下载仍须同时通过 catalog SHA-256 与官网 checksum。安装器已校验 bundle、运行依赖、逐文件摘要和 `networkRedirectPolicy.addressFamily=ipv4-only`，以版本目录加原子 managed symlink 提供 `/usr/local/lib/ppflight-agent/template-bootstrap`；helper 的镜像连接固定 `curl --disable --ipv4`、HTTPS-only redirect 和 catalog/official-checksum 完整性链。Agent 每次调用前再次校验，再以 `/usr/bin/python3 -I` 和受限环境执行唯一入口。真实 PVE 上会创建模板/备份的 plan/execute 尚未完成破坏性发布验收；它不新增 control action，更不表示 `vm.reinstall` 或远程 Agent 自升级已经实现。

...
func ipFilterDeleteRecoveryFixture(t *testing.T) (*Journal, Command, ipFilterDeleteRecoveryP, time.Time) {
	...
	failed.BodySHA256 = ipFilterDeletePayloadSHA256("ipfilter-net0", "74.91.18.94/32")
...
func TestIPFilterDeletePayloadSHA256MatchesWebsiteCanonicalJSON(t *testing.T) {
	const expected = "0811290452aa58df2e7d07c618fe0b967439d3aa2c14175b8a81f2c4771def93"
	if actual := ipFilterDeletePayloadSHA256("ipfilter-net0", "74.91.18.94/32"); actual != expected {
		t.Fatalf("payload digest=%q want %q", actual, expected)
	}
}
	...
	executor := Executor{
		...
		IPFilterDeleteJournal: ipFilterDeleteReconcilerFunc(func(_ Command, got ipFilterDeleteRecoveryP, present bool, observed string, _ time.Time) (IPFilterDeleteRecoveryResult, error) {
			...
			return IPFilterDeleteRecoveryResult{
				...
				ProviderReadVerified: true, TargetPresent: true, ObservedCIDR: observed,
			}, nil
		}),
	}

...
func TestQuickInstallFreshOnlyHostFirewallTransactionOrder(t *testing.T) {
	...
	ufwPreflight := strings.Index(quickInstall, "./ppflight-agent host-firewall prepare")
	...
	if classify < 0 || ufwPreflight < 0 || aptMutation < 0 || installMutation < 0 || prepare < 0 || lastServiceCheck < 0 || activate < 0 || reconcile < 0 || success < 0 {
		t.Fatalf("quick installer is missing host firewall lifecycle anchors")
	}
	if !(classify < ufwPreflight && ufwPreflight < aptMutation && classify < installMutation && installMutation < prepare && prepare < lastServiceCheck && lastServiceCheck < activate && activate < success) {

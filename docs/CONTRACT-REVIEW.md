...
- `ag-pve pve prepare/status` 是已接线的 root 本地 readiness 工具：prepare 先对固定、service 可读的 CA 与严格 DNS `pve.tlsServerName` 做无凭据 TLS 预检，再安全创建或读取 root-only Token；TCP endpoint 固定为 `127.0.0.1:8006/tcp4`。它要求 read Token 在 `/` 具备精确 audit 权限，并实际读取本机 node status/storage，随后切到 production/api、受控重启并等待真实采集及已绑定 telemetry 通道成功。它不会给 control Token 增授 ACL或打开 `productionExecution`；read readiness 不表示生产 mutation 验收完成。

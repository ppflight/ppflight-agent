...
control 当前 54 个 known actions 已用一致性测试锁住 registry/validator/executor。2026-09-02 起新增初次资源定型、固定模板重装、短时 console、只读 snapshot/backup、QEMU suspend/resume 与逐 NIC host-netdev 计量；2026-09-03 又新增一次性 VM legacy Journal 恢复、guest firewall rules list/get/verify，以及仅限 Linux QEMU、全 cluster 引用证明、digest 并发保护、durable phase/UPID 与双回读的 `vm.cloud-init-snippet.delete`。当前合同见 `PROVISIONING-ACTIONS-V1.md` 与 `AGENT-API-V1.md`。`vm.reinstall` 不接受本地 template helper、任意 URL/ISO/storage volume 或 shell，只接受 signed 固定模板 identity/config SHA 与完整交付回读合同；真实 PVE 生产开放仍须由外部控制面完成 action allowlist/审批和验收。
...
- `ag-pve pve prepare/status` 是已接线的 root 本地 readiness 工具：prepare 先对固定、service 可读的 CA 与严格 DNS `pve.tlsServerName` 做无凭据 TLS 预检，再安全创建或读取 root-only 的隔离 read/control Token；TCP endpoint 固定为 `127.0.0.1:8006/tcp4`。它要求 read Token 在 `/` 具备精确 audit 权限并实际读取本机 node status/storage，同时为 dedicated control role 自动补齐固定的 `/` VPS 权限、启用固定 loopback exporter 并回读验证，随后切到 production/api、受控重启并等待真实采集及已绑定 telemetry 通道成功。官网与独立 monitoring audit 双绑定稳定且 device identity 一致后自动打开 `productionExecution`；签名、assignment、审批、action allowlist 和审计仍是独立门槛。
...
- 官网修改类 command 还需要通过 monitoring 独立 HMAC 投递脱敏 audit event，使用独立 durable outbox/idempotency/sequence/timestamps；只允许冻结字段与 SHA-256 digest，不能保存或上传 secret、密码、Token、完整 parameters/result 或原始 UPID。VM/CT 可选目标投影严格限于 cluster/node/guestType/vmid/guestName；失败阶段严格限于 admission/policy/execution/receipt，监控端负责中文翻译。`RESOURCE_BUSY` 等已通过策略的本地准入失败必须表示为 `allowed/failed/admission`，不能误标 `denied`；未知时间省略，禁止 null 或伪造。wire schema 故意不含 `operationId/executionMode`，UPID 若出现只能是 digest。Agent wire/journal/outbox/runtime sink/uploader 已接线；监控存储/UI 仍由外部任务交付，是 production 修改动作的验收项。

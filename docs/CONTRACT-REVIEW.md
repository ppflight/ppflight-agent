...
- website/monitoring 每个 bind response 都必须有各自的 `networkPolicy={agentObservedIPv4,serverIPv4Allowlist}`：均为 canonical IPv4，allowlist 为 1..16 项。Agent 只对 URL hostname 的 `A ∩ allowlist` 作 tcp4 direct dial，仍保留 hostname 做 Host/TLS SNI/证书校验，并拒绝 proxy、redirect 与 IPv6。Agent strict decode/persistence/pinning 已接；两端服务的出口地址记录、policy 签发/轮换和部署仍待各自任务交付，两个 trust domain 不得共享 policy。

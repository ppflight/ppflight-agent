...
从 `0.1.1-rc.30` 起，命令接收、策略拒绝、执行开始、终态、耗时和自升级各复验阶段都会写入结构化 journal 日志。从 `0.1.1-rc.31` 起，root helper 的失败阶段与原因也会持久化并进入官网签名回执，且发布打包器生成的每个文件都会由自动测试逐项通过 helper 白名单。日志与回执只包含合同允许的 `source/stage/method/path/httpStatus/reason`，不记录参数、凭据、请求体、PVE 原始响应或 guest 输出。

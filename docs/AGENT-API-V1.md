...
| vm | `vm.migrate-legacy-journal` | 修改类、必须由当前有效 revision 签名审批、每个 clone lineage 仅能消费一次的本地恢复动作。只接受一个严格小于当前 revision 且与目标记录 audit 完全一致的 `legacyAssignmentRevision`、一个精确 legacy clone command/operation/digest、固定 templateRef/sourceVmid/source config SHA，以及显式排序的无 UPID indeterminate command ID 列表；命令 envelope 锁定当前 binding/device/epoch/VMID/generation，旧记录的 Agent/audit target/signing key/legacy assignment/resource key 及任何已存在 authority 字段必须一致。完整 authority 的历史记录与缺少新增列的旧 schema 都可进入逐字段校验，不能仅因字段完整而拒绝。若旧记录缺少 record-level action，只能从记录自身的有效 signed audit 恢复 known mutating VM action，clone 必须精确为 `vm.clone`。退休记录的 state 必须为 `indeterminate`、不得包含 UPID，code 仅接受 `EXECUTION_INDETERMINATE` 或历史 `PVE_RESULT_INDETERMINATE`。重新读取当前模板摘要后才补齐 legacy clone identity，并只标记退休列出的记录；绝不删除文件，不存在历史 revision 枚举或清空 Journal 参数。退休记录只有在 `RetiredByCommandID/RetiredAt` 完整合法、对应同资源同 authority 的迁移 Journal 已成功终态，且其 durable typed result 明确列出该 command 时才释放资源锁；部分标记、迁移未完成或结果不匹配继续 fail closed。Claim 冲突使用 `UNLISTED_ACTIVE_MUTATION`、`LISTED_RECORD_NOT_ELIGIBLE` 及六个脱敏 `CLONE_*` 细分码。 |
